          ...
          while ($row = $result->fetch_assoc()) {
            ...
            if ($field4name == 'MENTOR') {
              continue;
                ...
                $regno = $row["regno"];
                $eventcount_result = $link->query("select count(distinct event_name) from attendance")->fetch_assoc();
                $total_events = $eventcount_result['count(distinct event_name)'];
                $attended_events_row = $events_row_result = $link->query("select count(regno) from attendance where regno=" . $regno . " and attendance_status='P'")->fetch_assoc();
              ...
              echo '<tr> 
                        ...
                        <td class="align-middle">' . $att_percent . '%</td>
                        <td><button type="button" value=' . $field0name . '  name=' . $field0name . ' id="editbutton" class="btn btn-outline-primary mx-2">Edit</button>
